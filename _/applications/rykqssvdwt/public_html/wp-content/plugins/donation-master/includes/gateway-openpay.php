...
class Donation_Gateway_Openpay
{
    ...
    public function process_webhook_payment(){
        ...
        // $logfile = $path.'failure_'.date("Ymd").".log";
        ...
        //For manual test
        // $path = $this->config['plugin_dir_path']."includes/logs/failure.log";
        ...
        {
            $this->_wh_process_3d_success_secure_payment($response);
        }
        else if($response->type=='charge.failed' && $response->transaction->status=='failed')
        {
            $this->_wh_process_3d_failed_secure_payment($response);
        }
        else{
    ...
    public function add_payment($donor_id='', $response = SomeObject::class)
    {
        ...
        $donate_page_url ='';
        if(isset($_POST['donate_page_url']))
        {
            $donate_page_url = $_POST['donate_page_url'];            
        }

        $data = array('donor_id'=>$donor_id,'order_id'=>$order_id,'type'=>$type,'subscription_plan'=>$subscription_plan,'amount'=>$amount,'payment_date'=>$time,'status'=>'Pending','donation_from_page_url'=>$donate_page_url,'created_at'=>$time);    
        $format =  array();        
    ...
    public function update_payment_history($order_id='',$response = SomeObject::class)
    {
        ...
        if($response->error_code || $response->type=='charge.failed')
        {
            $status = 'failed'; 
        } 
        else
    ...
    public function update_payment_log($order_id='',$response = SomeObject::class)
    {
        ...
        if($response->error_code || $response->type=='charge.failed')
        {
            ...
            if($response->transaction->error_message)
            {
                $error_msg = $response->transaction->error_message;     
            }
        ...
        $time = current_time( 'mysql' );
        $final_data = array_merge($data,array('status'=>$status,'error_code'=>$error_code,'error_msg'=>$error_msg,'modified_at'=>$time));    
        $format =  array();
   ...
   public function get_customer_id($donor_id='',$subscription_id)
    ...
    public function get_payment_log($transaction_id='', $order_id='')
    {
        ...
        $payment_data = $wpdb->get_results($wpdb->prepare("SELECT dpl.order_id, dpl.donor_id, dd.name, dd.email, dpl.amount ,dpl.status,dpl.error_code,dpl.error_msg FROM $table_name as dpl left join $table_name1 as dd on dd.id = dpl.donor_id WHERE (transaction_id = '%s' or order_id = '%s') ",$transaction_id,$order_id));
       ...
       $output =array();
        ...
        {   
            ...
            do {                
                ...
                sleep(1);                
            } while ( (time() - $starttime <= 5) && $payment_data->status=='charge_pending');            
            ...
            {   
                ...
                {
                    $msg = $this->handleRequestError($payment_data->error_code);                
                }
                
                if($payment_data->status)
                {
                    $error_status = $this->handleResponseStatus($payment_data->status);                
                }
                
                if($payment_data->error_msg)
                {
                    $error_msg = $payment_data->error_msg;                
                }

                $msg = $error_msg .' ' .$msg.', '.$error_status; 
                $log = "Payment is failed due to :".
                $this->write_log($payment_data->email,$log);              
                $redirect_params.='&error='.$msg;
                $output =  array(
                    'status' => 'error',
                    'redirect' => $redirect_url . '?payment_status=fail'.$redirect_params
                );
   ...
   public function _wh_process_3d_success_secure_payment($response)
   {    
   ...
   public function _wh_process_3d_failed_secure_payment($response)
   {  
        ...
        {               
            ...
            {               
                ...
                if(!empty($response->transaction->error_message))
                {
                    $msg = $response->transaction->error_message;                    
                }
                else
                {
                    $msg = $this->handleResponseStatus('payout_off');   
                    $response->transaction->error_message='payout_off';
                ...
                $log = "Sent payment_failure mail";
                $this->write_log($payment_data->email,$log);  

                $log = "End: ". date('Y-m-d H:i:s');
                $this->write_log($payment_data->email,$log);  

                $return_data=array('status'=>'error','msg'=>"El pago ha fallado. No:".$response->transaction->order_id);
                echo json_encode($return_data);
                exit;
            }
        }
