...
{	
	...
	if($search)
  	...
  	$query="SELECT name,email,subscription_plan,amount,payment_date,dph.status,dph.donation_from_page_url,dph.order_id FROM $table_name as dph left join $table_name1  as dd on dd.id=dph.donor_id";
	...
	$output=array();
  	...
  	foreach ($paymentsSet as $key => $payment) {
		...
		$donor_id = $donorClass->get_donor_id_by_email($payment->email);			  				  		
  		...
  		//$name = $payment->name;
        if($payment->donation_from_page_url !='')
        {
            $ex_pid = explode("_",$payment->donation_from_page_url);
            if($ex_pid[0] == "pg")
            {
                $donation_page_from = get_the_title($ex_pid[1]);
                $donation_page_from_url = get_permalink($ex_pid[1]);    
            }
            else if($ex_pid[0] == "cat")
            {
                $donation_page_from = get_cat_name($ex_pid[1]);
                $donation_page_from_url = get_category_link($ex_pid[1]);       
            }
        
            $donation_from_page_url = "<a target='_blank' href=".$donation_page_from_url.">".$donation_page_from."</a>";
        }
        else
        {
           $donation_from_page_url = "<a target='_blank' href=".site_url().">Home</a>"; 
        }

  		if($payment->status=='completed'){$payment->status='Completed';}
	  	$output[] = array($name,$payment->email,$payment->order_id,$payment->amount,$payment_date,$payment->status,$donation_from_page_url);
...
{	
	...
	global $wpdb;
  	...
  	$query="SELECT donor_id,name,email,subscription_plan,amount,payment_date,next_payment_date,dph.status,dph.donation_from_page_url,dph.order_id FROM $table_name as dph left join $table_name1  as dd on dd.id=dph.donor_id";
	...
	$output=array();
  	foreach ($paymentsSet as $key => $payment) {
        ...
        else
        {
  		...
  		if($payment->status=='completed'){$payment->status='Completed';}
	  	$output[] = array($name,$payment->email,$payment->order_id,$payment->amount,$payment->subscription_plan,$payment_date,$next_payment_date,$payment->status,$donation_from_page_url);
...
function donation_install() {
	...
	$sql_2 = " CREATE TABLE IF NOT EXISTS `$table_name` (				
				`id` int(11) unsigned NOT NULL AUTO_INCREMENT,
				`order_id` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '0',
				`donor_id` int(11) NOT NULL,
				`type` enum('Subscription','One Time') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'One Time',
				`subscription_plan` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
				`amount` decimal(10,2) DEFAULT NULL,
				`payment_date` datetime DEFAULT NULL,
				`next_payment_date` datetime DEFAULT NULL,
                `status` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
				`donation_from_page_url` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
				`created_at` datetime DEFAULT NULL,
				`modified_at` datetime DEFAULT NULL,
				PRIMARY KEY (`id`),
	...
	$sql_4 =" CREATE TABLE IF NOT EXISTS `$table_name` (			
			`id` int(11) unsigned NOT NULL AUTO_INCREMENT,
			`order_id` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT '0',
			`donor_id` int(11) NOT NULL,
			`subscription_id` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
			`transaction_id` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
			`request_id` varchar(150) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
...
function datatables_onetime_delete() {
	...
	$table_name = $wpdb->prefix . 'donation_donors';
	$sql_1 = " Delete from `$table_name` where  created_at > '2018-10-27 00:00:00' ;  ";

	$table_name = $wpdb->prefix . 'donation_payment_history';	
	$sql_2 = " Delete from `$table_name` where  created_at > '2018-10-27 00:00:00' ; ";

	$table_name = $wpdb->prefix . 'donation_payment_log';
 	$sql_3 = " Delete from `$table_name` where  created_at > '2018-10-27 00:00:00' ;  ";  
