...
class ESSBSocialFollowersCounterUpdater {
	...
	public function update_instagram() {
		...
		$username = ESSBSocialFollowersCounterHelper::get_option ( 'instgram_username' );
		$api_key = ESSBSocialFollowersCounterHelper::get_option ( 'instgram_api_key' );
		// FB ID connected with Instagram business account
		$fb_page_id = "137667156253592";
		...
		try {
			$response = $this->remote_update ( 'https://graph.facebook.com/v3.2/' . $fb_page_id . '?fields=name,instagram_business_account{username,followers_count}&access_token=' . $api_key );

			if (isset ( $response['instagram_business_account'] ) && isset ( $response['instagram_business_account']['followers_count'] )) {
				return $response['instagram_business_account']['followers_count'];
			}
		} catch ( Exception $e ) {
			return 0;
		}
