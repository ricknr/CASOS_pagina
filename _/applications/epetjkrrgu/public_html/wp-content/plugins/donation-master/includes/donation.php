...
if ( !class_exists( 'Donation' ) ) {
	...
	require_once 'donation_views.php';
	...
	class Donation {
		...
		public function render()
		{
			...
			if($page=='plugin-donation')
			{

				if(isset($_GET['donor_id']) && $_GET['donor_id']!='')
				{
					$this->plugin_donation_donors();					
				}
				else
				{
					$this->plugin_donation_settings();
				}
				
			}
			

			if($page=='plugin-donation-emails')
			{
				$this->plugin_donation_emails();
			}
		}

		public function plugin_donation_donors()
		{
			// // Prepare page options			
			$options =array();
			// Get current page slug
			$page = sanitize_key( $_GET['page'] );
			$donor_id = sanitize_key( $_GET['donor_id'] );
			echo '<div id="donation-donors" class="wrap donation">';
			echo '<h1 class="wp-heading-inline" >'.$this->config['pages'][$page]['page_title'].'</h1>';			
			echo '<div class="custom_content_body">';	
			echo call_user_func( array( $this->config['views_class'], 'donor_personal_info' ), $options, $this->config );			
			echo '</div>';
			
		}

		public function plugin_donation_emails()
		{
			// // Prepare page options
			...
			$page = sanitize_key( $_GET['page'] );
			$donor_id = sanitize_key( $_GET['donor_id'] );
			echo '<div id="donation-email" class="wrap donation">';
			echo '<h1 class="wp-heading-inline" >'.$this->config['pages'][$page]['page_title'].'</h1>';			
			echo '<div class="custom_content_body">';	
			echo call_user_func( array( $this->config['views_class'], 'donation_emails' ), $options, $this->config );			
			echo '</div>';
			
		}

		public function plugin_donation_settings()
		{
			// Prepare page options
			$options = $this->get_page_options();
		...
		public function assets() {
			...
			// Register styles
			wp_register_style(  'donation-' . $this->config['version'], plugins_url( $this->config['css'], $this->config['file'] ) . '/donation.css', false, $this->config['version'], 'all' );

			wp_register_style(  'donation-datatable' , plugins_url( $this->config['assets'], $this->config['file'] ) . '/DataTables-1.10.18/datatables.min.css', false, $this->config['version'], 'all' );			
			
			// Register scripts
			wp_register_script( 'donation-' . $this->config['version'], plugins_url( $this->config['js'],  $this->config['file'] ) . '/donation.js', array( 'jquery', 'jquery-form' ), $this->config['version'], true );
			

			wp_register_script( 'donation-' . $this->config['version'], plugins_url( $this->config['js'],  $this->config['file'] ) . '/donation.js', array( 'jquery', 'jquery-form' ), $this->config['version'], true );
			
			wp_register_script( 'donation-datatable', plugins_url( $this->config['assets'],  $this->config['file'] ) . '/DataTables-1.10.18/datatables.min.js', array( 'jquery', 'jquery-form' ), $this->config['version'], true );
		...
		public function enqueue() {
			...
			// Enqueue styles
			foreach ( array( 'farbtastic', 'donation-' . $this->config['version'],'donation-datatable','donation-tab' ) as $style ) wp_enqueue_style( $style );
			// Enqueue scripts
			foreach ( array( 'jquery', 'jquery-form', 'farbtastic', 'donation-' . $this->config['version'],'donation-datatable','donation-tab','donation_datatable_params' ) as $script ) wp_enqueue_script( $script );
