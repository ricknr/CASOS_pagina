	...
	class Donors{
		...
		{			
			...
			global $wpdb;
		  	...
		  	$query="SELECT name,email,order_id,subscription_plan,amount,payment_date,next_payment_date,dph.status,dph.donation_from_page_url FROM $table_name as dph left join $table_name1  as dd on dd.id=dph.donor_id";
            ...
            $donation_from_page_url = "<a target='_blank' href=".site_url().">Home</a>";
		  	...
		  	{		  		
                ...
                if($donor->donation_from_page_url !='')
                {
                    $ex_pid = explode("_",$donor->donation_from_page_url);
                    if($ex_pid[0] == "pg")
                    {
			  	...
			  	$output[] = array($donor->name,$donor->email,$donor->order_id,$donor->amount,$payment_date,$donor->subscription_plan,$next_payment_date,$donor->status,$donation_from_page_url);
